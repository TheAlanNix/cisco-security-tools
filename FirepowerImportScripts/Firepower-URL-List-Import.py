...
# -------
#
# This script will take in a CSV list of URLs an then import them into Firepower Management Center.
#
# Requirements
# ------------
#
...
# ----------
#
#	1) Configure the CSV_URL_FILE variable 
...
# Logging Parameters
logging.basicConfig(filename='Firepower-URL-List-Import.log', format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', filemode='w', level=logging.INFO)

# CSV Filename
CSV_URL_FILE = "Example.csv"
...
if __name__ == "__main__":
	...
	logging.info('Loading CSV file: ' + CSV_URL_FILE)

	# Load in the CSV of URLs
	with open(CSV_URL_FILE) as csv_file:
	...
	logging.info('Loaded ' + str(line_count) + ' URLs.')

	# Ask the user for a name for the URL Group
	GROUP_NAME = raw_input("Enter URL Group Name: ")

	# Format the data to be sent to the FMC
	URL_OBJECT_JSON = {
		'type': 'UrlGroup',
		'literals': URL_OBJECTS,
		'overridable': True,
		'description': '',
		'name': GROUP_NAME,
	}

	logging.info(json.dumps(URL_OBJECT_JSON))

	logging.info('Starting Network Object Import...')

	# If not hard coded, get the FMC IP, Username and Password
	if not FMC_IP:
		FMC_IP = raw_input("FMC IP Address: ")
	if FMC_USERNAME is None:
		FMC_USERNAME = raw_input("FMC Username: ")
	...
	ObjectCallFMC('POST', 'urlgroups', URL_OBJECT_JSON)
	exit()